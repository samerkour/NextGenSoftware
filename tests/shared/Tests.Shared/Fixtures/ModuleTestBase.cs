    ...
    where TEntryPoint : class
{
    ...
    public ModuleTestBase(IntegrationTestFixture<TEntryPoint> integrationTestFixture, ITestOutputHelper outputHelper)
    {
        ...
        _checkpoint = new Checkpoint
        {
            ...
            DbAdapter = DbAdapter.SqlServer, TablesToIgnore = new List<Table> {new("__EFMigrationsHistory"),}.ToArray()
        };
    ...
    private async Task ResetState()
    {
        try
        {
            var SqlServerOptions = ModuleFixture.ServiceProvider.GetService<IOptions<SqlServerOptions>>();
            if (SqlServerOptions is { } && !string.IsNullOrEmpty(SqlServerOptions.Value.ConnectionString))
            {
                await using var conn = new SqlConnection(SqlServerOptions.Value.ConnectionString);
